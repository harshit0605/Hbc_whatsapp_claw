...
app = FastAPI(title="society-skills HTTP API", version="0.1.0")

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # portal runs on the same VPS or behind the same reverse proxy
...
def require_token(authorization: Annotated[str | None, Header()] = None) -> None:
    expected = get_settings().skills_http_internal_token
...
async def on_startup() -> None:
    ...
    if boot_email and boot_pw:
        async with session_scope() as s:
            if await repo.count_admins(s) == 0:
...
async def auth_verify(payload: dict[str, str]) -> dict[str, Any]:
    """Verify admin email + password; returns minimal admin profile."""
    require_token(payload.get("_authorization"))  # not used; we re-auth below
    raise HTTPException(status.HTTP_400_BAD_REQUEST, "use /auth/login")
...
async def dispatch(complaint_id: UUID, payload: dict[str, Any]) -> AssignmentOut:
    ...
    async with session_scope() as s:
        ...
        worker = next(
            (w for w in await repo.list_workers_by_category(s, limit=1000) if str(w.id) == worker_id),
            None,
        )
        if worker is None or not worker.is_active:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "worker not found or inactive")
    ...
    media_keys = [m.storage_key for m in complaint.media if m.kind == "image"]
    try:
        await send_text(
            to_jid=worker.wa_jid,
            body=body,
            media_storage_keys=media_keys or None,
        )
...
def _format_worker_dispatch(c: ComplaintOut, w: WorkerOut) -> str:
    ...
    return (
        ...
        f"{c.description}\n\n"
        f"Reply ACCEPT to take it, DONE when finished, or HELP if blocked."
    )
...
# ─── Media upload (used by the agent's media_handler plugin) ────────────────


@app.post("/media", dependencies=[AuthDep])
async def upload_media(file: UploadFile, kind: str = "image") -> dict[str, str]:
...
async def stats() -> dict[str, Any]:
    async with session_scope() as s:
        return {"by_status": await repo.count_complaints_by_status(s)}


# ─── Test alert (manual button for admins to verify configuration) ──────────


@app.post("/message-log", dependencies=[AuthDep])
async def post_message_log(payload: dict[str, Any]) -> GenericOk:
    direction = payload.get("direction")
    wa_jid = payload.get("wa_jid")
    if direction not in ("in", "out") or not wa_jid:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "direction and wa_jid required")
    async with session_scope() as s:
        # Best-effort resolution of resident/worker by JID
        resident = await repo.find_resident_by_jid(s, wa_jid=wa_jid)
        worker = None if resident else await repo.find_worker_by_jid(s, wa_jid=wa_jid)
        await repo.log_message(
            s,
            direction=direction,
            wa_jid=wa_jid,
            body=payload.get("body"),
            media_keys=payload.get("media_keys") or [],
...
async def _notify_resident_status_change(c: ComplaintOut) -> None:
    ...
    body = (
        ...
        f"Status: {pretty}.\n"
        f"Reply 'status #{c.ticket_no}' anytime to check again."
    )
